...
const SAVE_KEY = 'rail-rush-hour-v018';
const LEGACY_SAVE_KEY = 'rail-rush-hour-v017';
const TICK_MS = 1000;
...
function PersonSvg({ x, y, color = '#567c90' }) {
  return <G><Circle cx={x} cy={y} r="2" fill="#efc79d" /><Rect x={x - 2} y={y + 2} width="4" height="6" rx="1.2" fill={color} /></G>;
}
...
function ZoneMeter({ x, y, title, value, max, hot, accent = '#58b9ee' }) {
  ...
  return (
    <G>
      <Rect x={x} y={y} width="102" height="31" rx="6" fill="rgba(9,22,28,0.92)" stroke={hot ? '#ff735e' : '#617982'} strokeWidth={hot ? 2 : 0.9} />
      <SvgText x={x + 7} y={y + 10} fontSize="5.5" fontWeight="900" fill="#9bb0b8">{title}</SvgText>
      <SvgText x={x + 7} y={y + 23} fontSize="8.5" fontWeight="900" fill="#eff5f6">{value}/{max}</SvgText>
      <Rect x={x + 51} y={y + 18} width="43" height="4" rx="2" fill="#263a42" />
      <Rect x={x + 51} y={y + 18} width={43 * p / 100} height="4" rx="2" fill={hot ? '#ff735e' : accent} />
    </G>
  );
}
  ...
  return (
    <View style={styles.worldCanvas}>
      <Svg width={WORLD_WIDTH} height={WORLD_HEIGHT} viewBox={`0 0 ${WORLD_WIDTH} ${WORLD_HEIGHT}`}>
        ...
        <Polygon points="0,1040 0,600 1500,245 1500,1040" fill="#9fbd82" />

        {/* Rail corridor: a single continuous back edge of the station. */}
        <Polygon points="520,565 1365,360 1450,420 605,625" fill="#738667" opacity="0.62" />

        {/* Three platforms share the same station throat and concourse. */}
        {[1, 2, 3].map((lane) => {
          ...
          return (
            <G key={lane} opacity={locked ? 0.5 : 1}>
              <Polygon points={isoStrip(PLATFORM_X, y, PLATFORM_LENGTH, 54)} fill={locked ? '#8b8273' : '#aaa89d'} stroke={hot ? '#ff735e' : '#e3e0d5'} strokeWidth={hot ? 3 : 1.4} />
              <Polygon points={isoStrip(PLATFORM_X + 9, y + 45, PLATFORM_LENGTH, 24)} fill="#273236" />
              <Line x1={PLATFORM_X + 18} y1={y + 53} x2={PLATFORM_X + PLATFORM_LENGTH} y2={y + 53 - (PLATFORM_LENGTH - 18) * ISO} stroke="#bcc5c8" strokeWidth="2.4" />
              <Line x1={PLATFORM_X + 27} y1={y + 64} x2={PLATFORM_X + PLATFORM_LENGTH + 9} y2={y + 64 - (PLATFORM_LENGTH - 18) * ISO} stroke="#bcc5c8" strokeWidth="2.4" />
              <Rect x={PLATFORM_X + 12} y={y - 4} width="40" height="23" rx="4" fill="#0a171d" />
              ...
              {!locked && Array.from({ length: Math.min(5, 1 + Math.floor(platformLevel / 2)) }).map((_, i) => {
                const px = PLATFORM_X + 110 + i * 118;
                const py = y - 15 - i * 16.5;
                return <G key={i}><Line x1={px} y1={py + 32} x2={px} y2={py + 9} stroke="#4d5d62" strokeWidth="3" /><Line x1={px - 26} y1={py + 9} x2={px + 43} y2={py - 1} stroke="#66777b" strokeWidth="5" /></G>;
              })}
        ...
        {/* One continuous concourse spans station and platform heads. */}
        <Polygon points={isoStrip(515, 505, 360, 90)} fill="#7d8b8e" stroke="#dce4e5" strokeWidth="1.5" />
        <Polygon points={isoStrip(570, 470, 250, 62)} fill="#939d9d" stroke="#e3e8e7" strokeWidth="1" />
        <SvgText x="610" y="507" fontSize="7" fontWeight="900" fill="#f1f7f8">CENTRALE PERRONPASSAGE</SvgText>

        {/* Main station is one building attached directly to the concourse. */}
        <Polygon points={isoStrip(430, 575, hallWidth, 165)} fill="#526e7a" stroke={bottleneck === 'HAL' ? '#ff735e' : '#d8e5e9'} strokeWidth={bottleneck === 'HAL' ? 3.2 : 1.5} />
        <Polygon points={`${430},575 548,660 548,795 430,710`} fill="#344b55" />
        <Polygon points={`${548},660 ${430 + hallWidth + 165},${575 - hallWidth * ISO + 119} ${430 + hallWidth + 165},${710 - hallWidth * ISO + 119} ${548},795`} fill="#405b66" />
        <Polygon points={isoStrip(450, 557, hallWidth - 42, 125)} fill="#263a44" />
        <SvgText x="560" y="638" fontSize="14" fontWeight="900" fill="#f1f7f9">CENTRAAL STATION</SvgText>
        <SvgText x="562" y="655" fontSize="6.5" fontWeight="800" fill="#bed3db">HAL Lv {hallLevel} • SERVICE Lv {ticketLevel}</SvgText>
        {Array.from({ length: 9 }).map((_, i) => <Rect key={i} x={560 + i * 28} y={688 - i * 4} width="14" height="22" fill="#7ebad2" stroke="#d1e9f1" strokeWidth="0.8" />)}

        {/* Gates are part of the station frontage, not a separate zone. */}
        <Polygon points={isoStrip(410, 727, 235, 54)} fill="#344950" stroke={bottleneck === 'ENTREE' ? '#ff735e' : '#718993'} strokeWidth={bottleneck === 'ENTREE' ? 2.8 : 1.1} />
        {Array.from({ length: gates }).map((_, i) => {
          const gx = 438 + i * 19;
          const gy = 750 - i * 2.7;
          return <G key={i}><Rect x={gx} y={gy} width="8" height="23" fill="#536d76" stroke="#a3b7be" strokeWidth="0.7" /><Circle cx={gx + 4} cy={gy + 5} r="1.5" fill="#50e18a" /></G>;
        })}
        <ZoneMeter x={455} y={800} title="ENTREE" value={entranceQueue} max={entranceBuffer(gateLevel)} hot={bottleneck === 'ENTREE'} accent="#ffd25e" />

        {/* Hall interior continues behind the entrance and into the concourse. */}
        {Array.from({ length: Math.min(36, Math.ceil(hallTotal / 7)) }).map((_, i) => <PersonSvg key={i} x={565 + (i % 12) * 21 + Math.floor(i / 12) * 7} y={760 - (i % 12) * 3 + Math.floor(i / 12) * 15} color={i % 5 === 0 ? '#d09547' : '#507f9a'} />)}
        <ZoneMeter x={735} y={760} title="STATIONSHAL" value={hallTotal} max={hallCap(hallLevel)} hot={bottleneck === 'HAL'} accent="#64db93" />

        {/* Retail is built into the station frontage. */}
        {Array.from({ length: kioskCount }).map((_, i) => <G key={i}><Polygon points={isoStrip(650 + i * 51, 805 - i * 7.2, 37, 22)} fill={i % 2 ? '#795a37' : '#6d4631'} stroke="#d2b27a" strokeWidth="0.8" /><SvgText x={657 + i * 51} y={815 - i * 7.2} fontSize="5" fontWeight="900" fill="#ffe2b0">{i % 2 ? 'SHOP' : 'CAFE'}</SvgText></G>)}

        {/* One forecourt touches the entire station front. */}
        <Polygon points={isoStrip(275, 795, 625, 175)} fill="#c6c2b6" stroke="#e3e0d7" strokeWidth="1.2" />
        <Polygon points={isoStrip(390, 790, 350, 78)} fill="#d1cdc2" stroke="#ebe7de" strokeWidth="1" />
        <SvgText x="515" y="826" fontSize="7" fontWeight="900" fill="#5a676b">STATIONSPLEIN</SvgText>

        {/* Parking is a wing of the same forecourt. */}
        <Polygon points={isoStrip(65, 820, 350 + Math.min(75, parkingLevel * 12), 170)} fill="#8d9899" stroke={bottleneck === 'PARKEREN' ? '#ff735e' : '#d7dddd'} strokeWidth={bottleneck === 'PARKEREN' ? 3.2 : 1.2} />
        {Array.from({ length: parkingSlots }).map((_, i) => {
          ...
          return <G key={i}><Polygon points={`${x},${y} ${x + 31},${y - 4.4} ${x + 37},${y + 1} ${x + 6},${y + 5.5}`} fill="none" stroke="#dde2e0" strokeWidth="0.75" />{i < occupiedCars ? <CarSvg x={x + 8} y={y} color={i % 4 === 0 ? '#ed6d62' : i % 4 === 1 ? '#4aa8ff' : i % 4 === 2 ? '#f0c64f' : '#e8eef0'} /> : null}</G>;
        })}
        <ZoneMeter x={245} y={965} title="PARKEREN" value={parkingQueue} max={parkingCap(parkingLevel)} hot={bottleneck === 'PARKEREN'} />

        {/* Access road feeds the same forecourt. */}
        <Polygon points="0,995 315,900 430,932 92,1035" fill="#596365" />
        <Line x1="25" y1="1000" x2="392" y2="892" stroke="#e1e4df" strokeWidth="2" strokeDasharray="12 11" opacity="0.58" />

        {[65, 145, 1040, 1120, 1320, 1400].map((x, i) => <Tree key={i} x={x} y={i < 2 ? 780 - i * 45 : 820 - (i - 2) * 58} scale={0.72 + (i % 3) * 0.08} />)}
      </Svg>

      {/* Visible walking chain follows one physical route through the station. */}
      ...
      {[1, 2, 3].map((lane) => {
        ...
        return (
          <React.Fragment key={lane}>
            <View pointerEvents="none" style={[styles.platformCrowd, { top: LANE_Y[lane] - 3 }]}>{Array.from({ length: Math.min(26, Math.ceil(waiting / 9)) }).map((_, i) => <View key={i} style={styles.platformPerson}><View style={[styles.platformHead, i % 4 === 0 && { backgroundColor: train?.destination?.color || '#58b9ee' }]} /><View style={styles.platformBody} /></View>)}</View>
            {train && !hidden ? <View style={[styles.trainAtPlatform, { top: LANE_Y[lane] + 42 }]}><TrainSprite train={train} ready={ready} late={late} doorsOpen={train.status === 'dwelling'} onPress={() => onDepart(lane)} /><Text style={[styles.trainStatus, ready && styles.readyText, late && styles.lateText]}>{train.number} → {train.destination.name} • {train.onboard}/{train.capacity} • {train.status === 'dwelling' ? `${train.remaining}s halte` : depIn > 0 ? `vertrek over ${depIn}s` : depIn >= -DELAY_MARGIN ? `VERTREK • ${DELAY_MARGIN + depIn}s marge` : `+${Math.abs(depIn + DELAY_MARGIN)}s te laat`}</Text></View> : null}
          </React.Fragment>
        );
      })}
...
export default function AppV18() {
  ...
  const begin = () => {
    ...
    setMessage('Eén stationscomplex: voorplein → entree → hal → passage → perrons → trein.');
    setPhase('playing');
  ...
  const doUpgrade = (kind) => {
    const map = {
      ...
      platforms: [platformCost(platformLevelRef.current), platformLevelRef, setPlatformLevel, 'Perroncapaciteit van het stationscomplex verbeterd.'],
    ...
    return (
      <SafeAreaView style={styles.screen}>
        ...
        <View style={styles.menu}>
          <Text style={styles.kicker}>SINGLE STATION LAYOUT / V0.18</Text>
          <Text style={styles.logo}>RAIL{`\n`}RUSH HOUR</Text>
          <Text style={styles.subtitle}>Eén stationscomplex waarin alle zones fysiek in elkaar passen: voorplein, hoofdingang, hal, perronpassage en perrons vormen samen één gebouwde locatie.</Text>
...
const styles = StyleSheet.create({
  ...
  hudWarn: { color: '#ffca62', fontSize: 7.4, fontWeight: '900', marginTop: 4 },
  ...
  cameraText: { color: '#bdd4df', fontSize: 5.7, fontWeight: '900' },
  ...
  platformCrowd: { position: 'absolute', left: PLATFORM_X + 78, width: 500, minHeight: 42, zIndex: 18, flexDirection: 'row', flexWrap: 'wrap', gap: 4, transform: [{ rotateZ: '-8deg' }] },
  ...
  upgrade: { width: 138, minHeight: 88, backgroundColor: '#e9eff1', borderWidth: 2, borderColor: '#486375', borderRadius: 9, padding: 7, flexDirection: 'row', alignItems: 'center', gap: 6 },
  ...
  upgradeDesc: { color: '#647985', fontSize: 5.3, marginTop: 2, fontWeight: '700' },
  upgradeCost: { color: '#2f7a43', fontSize: 5.8, fontWeight: '900' },
});