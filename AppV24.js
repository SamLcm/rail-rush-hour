...
// Developer reference build. All code/art/layout here is original placeholder work.
// Core test: bottlenecks, route-specific demand, rush hour and manual departure decisions.

const SAVE_KEY = 'rail-rush-hour-v024';
const LEGACY_KEY = 'rail-rush-hour-v023';
const TICK = 1000;
const WORLD_W = 1320;
const WORLD_H = 900;
const MAX_LEVEL = 10;
...
const STAGES = [
  ['Small Station', 'Parking • Tickets • Security • Platform 1'],
  ['Local Hub', 'Café • groter voorplein'],
  ['Town Station', 'Shop • Lakeside route'],
  ['Growing Junction', 'Platform 2 • 2-delige trein'],
  ['Regional Station', 'Toilets • Airport route'],
  ['Regional Hub', 'Restaurant • grotere hal'],
  ['Major Station', 'VIP • Harbor route • 3-delige trein'],
  ['Intercity Hub', 'Platform 3 • premium capaciteit'],
  ['Metropolitan', 'Capital route • groot terminalgebouw'],
  ['Grand Terminal', 'Manager automation • volledige operatie'],
];
...
function RouteDemand({ route, value, active }) {
  return <View style={[styles.routeChip,active&&styles.routeChipActive,{borderColor:route.color}]}><View style={[styles.routeDot,{backgroundColor:route.color}]}/><Text style={styles.routeChipText}>{route.name} {value}</Text></View>;
}
...
export default function AppV24(){
  ...
  const [cash,setCash]=useState(saved?.cash??750);
  ...
  const emptyDemand=Object.fromEntries(ROUTES.map(r=>[r.id,0]));
  const [platformDemand,setPlatformDemand]=useState({...emptyDemand,greenfield:12});
  ...
  const [trainSets,setTrainSets]=useState(1);
  ...
  const route=unlockedRoutes[routeIndex%Math.max(1,unlockedRoutes.length)]||ROUTES[0];
  ...
  const cashRef=useRef(cash),levelsRef=useRef(levels),parkingRef=useRef(parkingQ),entranceRef=useRef(entranceQ),ticketRef=useRef(ticketQ),securityRef=useRef(securityQ),waitingRef=useRef(waitingQ),demandRef=useRef(platformDemand),onboardRef=useRef(onboard),servedRef=useRef(served),departuresRef=useRef(departures),lostRef=useRef(lost),satisfactionRef=useRef(satisfaction),trainPhaseRef=useRef(trainPhase),countdownRef=useRef(countdown),delayRef=useRef(delay),holdRef=useRef(hold),clockRef=useRef(clock);
  useEffect(()=>{cashRef.current=cash;},[cash]); useEffect(()=>{levelsRef.current=levels;},[levels]); useEffect(()=>{parkingRef.current=parkingQ;},[parkingQ]); useEffect(()=>{entranceRef.current=entranceQ;},[entranceQ]); useEffect(()=>{ticketRef.current=ticketQ;},[ticketQ]); useEffect(()=>{securityRef.current=securityQ;},[securityQ]); useEffect(()=>{waitingRef.current=waitingQ;},[waitingQ]); useEffect(()=>{demandRef.current=platformDemand;},[platformDemand]); useEffect(()=>{onboardRef.current=onboard;},[onboard]); useEffect(()=>{trainPhaseRef.current=trainPhase;},[trainPhase]); useEffect(()=>{countdownRef.current=countdown;},[countdown]); useEffect(()=>{delayRef.current=delay;},[delay]); useEffect(()=>{holdRef.current=hold;},[hold]); useEffect(()=>{clockRef.current=clock;},[clock]); useEffect(()=>{satisfactionRef.current=satisfaction;},[satisfaction]);
  ...
  const trainCapacity=baseTrainCap*trainSets;
  const totalPlatform=Object.values(platformDemand).reduce((a,b)=>a+b,0);
  const routeWaiting=platformDemand[route.id]||0;

  const pressures=[
    ['PARKING',parkingQ/parkingCap,[170,650]],['ENTRANCE',entranceQ/entranceCap,[330,520]],['TICKETS',ticketQ/ticketCap,[455,510]],['SECURITY',securityQ/securityCap,[575,510]],['WAITING',waitingQ/waitingCap,[720,510]],['PLATFORM',totalPlatform/platformCap,[760,185]],['TRAIN',onboard/trainCapacity,[930,100]],
  ].sort((a,b)=>b[1]-a[1]);
  ...
  const upgrade=kind=>{const spec=FACILITIES[kind];if(!spec||effectiveLevel<spec.unlock)return;const lv=levels[kind]||0;if(lv>=MAX_LEVEL)return;const price=upgradeCost(spec.base,Math.max(1,lv||1));if(!spend(price))return setMessage('Nog niet genoeg geld.');setLevels(prev=>({...prev,[kind]:lv+1}));setMessage(`${spec.title} → Lv ${lv+1}. Kijk welk nieuw knelpunt ontstaat.`);};

  const departNow=()=>{
    if(trainPhaseRef.current!=='ready')return;
    const demand={...demandRef.current}; const stranded=demand[route.id]||0;
    const currentOnboard=onboardRef.current; const late=delayRef.current;
    const gross=Math.round(currentOnboard*route.fare*(1+(Math.max(1,levelsRef.current.train)-1)*0.08));
    const operatingCost=35*trainSets; const punctualBonus=late<=3?120:late<=8?50:0; const payout=Math.max(0,gross-operatingCost+punctualBonus);
    addCash(payout); servedRef.current+=currentOnboard; departuresRef.current+=1; setServed(servedRef.current); setDepartures(departuresRef.current);
    const penalty=Math.min(10,stranded*.035)+Math.max(0,late-3)*.18; changeSatisfaction(-penalty+(late<=3?1.2:0));
    setMessage(`${route.name}: ${currentOnboard} mee • ${stranded} wachten • ${late}s vertraging • +${money(payout)}`);
    onboardRef.current=0;setOnboard(0);trainPhaseRef.current='away';setTrainPhase('away');countdownRef.current=4;setCountdown(4);delayRef.current=0;setDelay(0);holdRef.current=0;setHold(0);
  };
  const waitFive=()=>{if(trainPhaseRef.current!=='ready')return;holdRef.current=5;setHold(5);trainPhaseRef.current='holding';setTrainPhase('holding');setMessage('Trein wacht 5 seconden voor extra instappers. Dit telt als vertraging.');};

  const persist=()=>save({cash:cashRef.current,gems,levels:levelsRef.current,stationLevel,served:servedRef.current,departures:departuresRef.current,lost:lostRef.current,satisfaction:satisfactionRef.current});

  useEffect(()=>{
    if(phase!=='playing')return undefined;
    const id=setInterval(()=>{
      const lv=levelsRef.current; let pk=parkingRef.current,e=entranceRef.current,t=ticketRef.current,s=securityRef.current,w=waitingRef.current,o=onboardRef.current; let demand={...demandRef.current};
      ...
      const currentPlatformTotal=Object.values(demand).reduce((a,b)=>a+b,0);const toPlatform=Math.min(w,rate(5,Math.max(1,lv.waiting),4)+(platformsUnlocked-1)*5,Math.max(0,platformCap-currentPlatformTotal));w-=toPlatform;
      for(let i=0;i<toPlatform;i++){const r=unlockedRoutes[(c+i)%Math.max(1,unlockedRoutes.length)];demand[r.id]=(demand[r.id]||0)+1;}

      let phaseNow=trainPhaseRef.current,cd=countdownRef.current,dl=delayRef.current,h=holdRef.current;
      if(phaseNow==='boarding'||phaseNow==='holding'){
        const match=demand[route.id]||0;const board=Math.min(match,rate(9,Math.max(1,lv.platform),6),Math.max(0,trainCapacity-o));demand[route.id]=match-board;o+=board;
      }
      if(phaseNow==='boarding'){cd=Math.max(0,cd-1);if(cd<=0){phaseNow='ready';dl=0;setMessage(`Vertrektijd ${route.name}. Vertrek nu of wacht 5s voor ${demand[route.id]||0} reizigers.`);}}
      else if(phaseNow==='holding'){h=Math.max(0,h-1);dl+=1;if(h<=0)phaseNow='ready';}
      else if(phaseNow==='ready'){
        dl+=1;
        if((lv.manager||0)>0&&dl>=3){trainPhaseRef.current='ready';demandRef.current=demand;onboardRef.current=o;setPlatformDemand(demand);setOnboard(o);setTimeout(departNow,20);}
      } else if(phaseNow==='away'){cd=Math.max(0,cd-1);if(cd<=0){phaseNow='boarding';cd=Math.max(14,23-effectiveLevel);setRouteIndex(v=>(v+1)%Math.max(1,unlockedRoutes.length));}}
      ...
      parkingRef.current=pk;entranceRef.current=e;ticketRef.current=t;securityRef.current=s;waitingRef.current=w;demandRef.current=demand;onboardRef.current=o;trainPhaseRef.current=phaseNow;countdownRef.current=cd;delayRef.current=dl;holdRef.current=h;
      setParkingQ(pk);setEntranceQ(e);setTicketQ(t);setSecurityQ(s);setWaitingQ(w);setPlatformDemand(demand);setOnboard(o);setTrainPhase(phaseNow);setCountdown(cd);setDelay(dl);setHold(h);
      if((c%15)===0)persist();
    },TICK);return()=>clearInterval(id);
  },[phase,effectiveLevel,parkingCap,entranceCap,ticketCap,securityCap,waitingCap,platformCap,trainCapacity,routeIndex,trainSets]);

  if(phase==='menu')return <SafeAreaView style={styles.screen}><StatusBar barStyle="light-content"/><View style={styles.menu}><Text style={styles.devTag}>DEEPER GAMEPLAY PASS • V0.24</Text><Text style={styles.logo}>RAIL{`\n`}RUSH HOUR</Text><Text style={styles.menuText}>Dezelfde idle-stationbasis, maar nu met echte gevolgen: spitsdrukte, route-specifieke reizigers, tevredenheid, achterblijvers en zelf beslissen wanneer de trein vertrekt.</Text><Pressable style={styles.start} onPress={()=>{setPhase('playing');setMessage('Let op de keten én de vertrektijd. Een upgrade verplaatst vaak het knelpunt.');setTimeout(()=>jumpTo(620,475),60);}}><Text style={styles.startText}>OPEN STATION</Text></Pressable><Text style={styles.safeNote}>DEV REFERENCE • tijdelijke eigen graphics • release register actief</Text></View></SafeAreaView>;
  ...
  return <SafeAreaView style={styles.screen}><StatusBar barStyle="light-content"/>
    ...
    <View style={[styles.rushBar,rush&&styles.rushBarOn]}><Text style={styles.rushText}>{rush?'⚡ RUSH HOUR • INSTROOM +85%':'NORMALE DIENST • volgende spits over '+Math.max(0,55-(clock%75))+'s'}</Text><View style={styles.devControls}><Pressable style={styles.devBtn} onPress={()=>setDevLevel(v=>Math.max(1,(v??stationLevel)-1))}><Text style={styles.devBtnText}>−</Text></Pressable><Pressable style={styles.devBtnWide} onPress={()=>setDevLevel(null)}><Text style={styles.devBtnText}>LIVE</Text></Pressable><Pressable style={styles.devBtn} onPress={()=>setDevLevel(v=>Math.min(10,(v??stationLevel)+1))}><Text style={styles.devBtnText}>+</Text></Pressable></View></View>
    <View style={styles.operationBar}><View style={{flex:1}}><Text style={styles.opLabel}>{route.name.toUpperCase()} • P1 • {routeWaiting} WACHTEN</Text><Text style={styles.opMain}>{trainPhase==='ready'?`VERTREKTIJD • +${delay}s`:trainPhase==='holding'?`WACHT ${hold}s • +${delay}s` : trainPhase==='away'?`VOLGENDE TREIN ${countdown}s`:`VERTREK OVER ${countdown}s`} • {onboard}/{trainCapacity}</Text></View>{trainPhase==='ready'?<><Pressable style={styles.waitButton} onPress={waitFive}><Text style={styles.waitText}>+5s WACHT</Text></Pressable><Pressable style={styles.departButton} onPress={departNow}><Text style={styles.departText}>VERTREK</Text></Pressable></>:null}</View>
    ...
    <View style={styles.viewport} onLayout={e=>setViewport({width:e.nativeEvent.layout.width,height:e.nativeEvent.layout.height})} {...panResponder.panHandlers}>
      <Animated.View style={[styles.world,{transform:[{translateX:camera.x},{translateY:camera.y}]}]}><View style={styles.grass}/><View style={styles.road}><View style={styles.roadDash}/></View><View style={styles.plaza}/>
        ...
        <View style={styles.station}><View style={styles.roof}><Text style={styles.stationName}>CENTRAL VALLEY</Text><Text style={styles.stationMeta}>{currentStage[1]}</Text></View>
          <View style={[styles.entrance,bottleneck[0]==='ENTRANCE'&&styles.zoneHot]}><Text style={styles.zoneTitle}>ENTRANCE</Text><Text style={styles.zoneSub}>{entranceQ}/{entranceCap}</Text><People count={entranceQ}/></View>
          ...
          <View style={styles.services}><Text style={styles.servicesTitle}>SERVICES</Text><Text style={styles.servicesText}>☕{levels.cafe}  🛍️{levels.shop}  🚻{levels.toilet}  🍽️{levels.restaurant}  ⭐{levels.vip}</Text></View>
        </View>
        {[1,2,3].map((n)=>{const unlocked=n<=platformsUnlocked;const top=205-(n-1)*72;return <View key={n} style={[styles.platform,{top,opacity:unlocked?1:.28},bottleneck[0]==='PLATFORM'&&n===1&&styles.zoneHot]}><View style={styles.yellowLine}/><Text style={styles.platformTitle}>{unlocked?`PLATFORM ${n}`:`PLATFORM ${n} LOCKED`}</Text>{unlocked&&n===1?<View style={styles.routeChips}>{unlockedRoutes.map(r=><RouteDemand key={r.id} route={r} value={platformDemand[r.id]||0} active={r.id===route.id}/>)}</View>:<Text style={styles.zoneSub}>{unlocked?'open':'later level'}</Text>}</View>;})}
        {[1,2,3].map((n)=>{const top=105-(n-1)*72;return <View key={`t${n}`} style={[styles.track,{top,opacity:n<=platformsUnlocked?1:.22}]}>{Array.from({length:23}).map((_,i)=><View key={i} style={[styles.sleeper,{left:i*42}]}/>)}<View style={[styles.rail,{top:18}]}/><View style={[styles.rail,{top:48}]}/></View>;})}
        {trainPhase!=='away'?<View style={styles.trainArea}><View style={styles.trainShadow}/><View style={[styles.train,bottleneck[0]==='TRAIN'&&styles.trainHot]}><View style={[styles.trainFront,{backgroundColor:route.color}]}/><View style={styles.trainRoof}/><View style={styles.windows}>{Array.from({length:7}).map((_,i)=><View key={i} style={styles.window}/>)}</View><View style={styles.trainDoor}/><View style={styles.wheels}>{Array.from({length:4}).map((_,i)=><View key={i} style={styles.wheel}/>)}</View></View><View style={styles.trainInfo}><Text style={styles.trainRoute}>{route.name}</Text><Text style={styles.trainLoad}>{onboard}/{trainCapacity} • {trainSets} set{trainSets>1?'s':''}</Text><View style={styles.loadTrack}><View style={[styles.loadFill,{width:`${Math.round(clamp(onboard/trainCapacity,0,1)*100)}%`,backgroundColor:route.color}]}/></View><Text style={styles.trainState}>{trainPhase==='ready'?`READY +${delay}s`:trainPhase==='holding'?`HOLD ${hold}s`:`${countdown}s`}</Text></View></View>:null}
        <FacilityBubble x={95} y={690} kind="parking" level={levels.parking} stationLevel={effectiveLevel} cash={cash} onUpgrade={upgrade} hot={bottleneck[0]==='PARKING'}/><FacilityBubble x={420} y={585} kind="ticket" level={levels.ticket} stationLevel={effectiveLevel} cash={cash} onUpgrade={upgrade} hot={bottleneck[0]==='TICKETS'}/><FacilityBubble x={545} y={585} kind="security" level={levels.security} stationLevel={effectiveLevel} cash={cash} onUpgrade={upgrade} hot={bottleneck[0]==='SECURITY'}/><FacilityBubble x={680} y={585} kind="waiting" level={levels.waiting} stationLevel={effectiveLevel} cash={cash} onUpgrade={upgrade} hot={bottleneck[0]==='WAITING'}/><FacilityBubble x={790} y={255} kind="platform" level={levels.platform} stationLevel={effectiveLevel} cash={cash} onUpgrade={upgrade} hot={bottleneck[0]==='PLATFORM'}/><FacilityBubble x={955} y={70} kind="train" level={levels.train} stationLevel={effectiveLevel} cash={cash} onUpgrade={upgrade} hot={bottleneck[0]==='TRAIN'}/>
        <FacilityBubble x={820} y={590} kind="cafe" level={levels.cafe} stationLevel={effectiveLevel} cash={cash} onUpgrade={upgrade}/><FacilityBubble x={925} y={590} kind="shop" level={levels.shop} stationLevel={effectiveLevel} cash={cash} onUpgrade={upgrade}/><FacilityBubble x={1030} y={590} kind="toilet" level={levels.toilet} stationLevel={effectiveLevel} cash={cash} onUpgrade={upgrade}/><FacilityBubble x={1080} y={470} kind="restaurant" level={levels.restaurant} stationLevel={effectiveLevel} cash={cash} onUpgrade={upgrade}/><FacilityBubble x={1080} y={355} kind="vip" level={levels.vip} stationLevel={effectiveLevel} cash={cash} onUpgrade={upgrade}/><FacilityBubble x={1120} y={240} kind="manager" level={levels.manager} stationLevel={effectiveLevel} cash={cash} onUpgrade={upgrade}/>
      </Animated.View>
      <View style={styles.leftMenu}><SideButton icon="⚙️" label="Operate" onPress={()=>setPanel('operate')} badge={trainPhase==='ready'?'!':null}/><SideButton icon="📍" label="Routes" onPress={()=>setPanel('routes')}/><SideButton icon="📊" label="Flow" onPress={()=>setPanel('flow')}/><SideButton icon="🏗️" label="Levels" onPress={()=>setPanel('levels')}/></View><View style={styles.zoomButtons}><Pressable style={styles.zoomBtn} onPress={()=>jumpTo(620,475)}><Text>⌂</Text></Pressable><Pressable style={styles.zoomBtn} onPress={()=>jumpTo(...bottleneck[2])}><Text>⚠️</Text></Pressable><Pressable style={styles.zoomBtn} onPress={()=>jumpTo(760,160)}><Text>🚆</Text></Pressable></View><View style={styles.message}><Text style={styles.messageText}>{message}</Text></View>
      {panel==='operate'?<Panel title="OPERATIONS" onClose={()=>setPanel(null)}><Text style={styles.panelLead}>Treinlengte beïnvloedt capaciteit én kosten per vertrek.</Text><View style={styles.setRow}>{[1,2,3].map(n=><Pressable key={n} disabled={n>maxSets||trainPhase==='ready'||trainPhase==='holding'} onPress={()=>setTrainSets(n)} style={[styles.setButton,trainSets===n&&styles.setButtonActive,n>maxSets&&styles.setButtonLocked]}><Text style={styles.setText}>{n} set{n>1?'s':''}</Text><Text style={styles.setSub}>{n>maxSets?`Lv ${n===2?4:7}`:`cap ${baseTrainCap*n}`}</Text></Pressable>)}</View><Text style={styles.panelLead}>Vertrekkeuze:</Text><Text style={styles.listSub}>Op tijd vertrekken beschermt tevredenheid. Wachten kan extra reizigers opleveren, maar vertraging en achterblijvers tellen mee.</Text></Panel>:null}
      {panel==='routes'?<Panel title="ROUTE DEMAND" onClose={()=>setPanel(null)}>{ROUTES.map(r=><View key={r.id} style={[styles.listRow,effectiveLevel<r.unlock&&styles.lockedRow]}><View><Text style={styles.listTitle}>{r.name}</Text><Text style={styles.listSub}>Wachtend: {platformDemand[r.id]||0} • Fare {money(r.fare)} • unlock Lv {r.unlock}</Text></View></View>)}</Panel>:null}
      ...
      {panel==='levels'?<Panel title="STATION LEVELS" onClose={()=>setPanel(null)}><ScrollView>{STAGES.map((s,i)=><Pressable key={i} onPress={()=>setDevLevel(i+1)} style={[styles.listRow,effectiveLevel===i+1&&styles.listRowActive]}><View><Text style={styles.listTitle}>LEVEL {i+1} • {s[0]}</Text><Text style={styles.listSub}>{s[1]}</Text></View></Pressable>)}</ScrollView></Panel>:null}
    </View>
    <View style={styles.bottomStats}><Text style={styles.stat}>🚗 {parkingQ}/{parkingCap}</Text><Text style={styles.stat}>👥 {served} vervoerd</Text><Text style={styles.stat}>❌ {lost} verloren</Text><Text style={styles.stat}>🚆 {departures} vertrek</Text><Text style={styles.stat}>🙂 {satisfaction}%</Text></View>
  </SafeAreaView>;
}
...
const styles=StyleSheet.create({
  ...
  operationBar:{minHeight:54,backgroundColor:'#f3f0e4',paddingHorizontal:9,paddingVertical:7,flexDirection:'row',alignItems:'center',gap:6,borderBottomWidth:2,borderBottomColor:'#b8b4a8'},opLabel:{color:'#7d786b',fontSize:5.6,fontWeight:'900'},opMain:{color:'#263e49',fontSize:8.2,fontWeight:'900',marginTop:2},waitButton:{backgroundColor:'#d9a64c',paddingHorizontal:9,paddingVertical:9,borderRadius:7},waitText:{color:'#fff',fontSize:5.5,fontWeight:'900'},departButton:{backgroundColor:'#51b978',paddingHorizontal:11,paddingVertical:9,borderRadius:7},departText:{color:'#fff',fontSize:6,fontWeight:'900'},
  ...
  station:{position:'absolute',left:390,top:395,width:690,height:320,backgroundColor:'#eee9da',borderRadius:14,borderWidth:4,borderColor:'#476677'},roof:{position:'absolute',left:0,right:0,top:0,height:55,backgroundColor:'#365d70',borderTopLeftRadius:10,borderTopRightRadius:10,paddingHorizontal:12,paddingTop:9},stationName:{color:'#fff',fontSize:12,fontWeight:'900'},stationMeta:{color:'#abc8d4',fontSize:5.4,fontWeight:'800',marginTop:2},entrance:{position:'absolute',left:-92,top:128,width:92,height:118,backgroundColor:'#d6e0e2',borderWidth:3,borderColor:'#5e7d89',borderRadius:9,padding:6},room:{position:'absolute',top:76,height:205,backgroundColor:'#f5f0e2',borderWidth:2,borderColor:'#a7aa9d',borderRadius:8,padding:6},ticket:{left:18,width:120},security:{left:147,width:120},waiting:{left:276,width:190},services:{position:'absolute',left:475,top:76,width:195,height:205,backgroundColor:'#e7ddcc',borderWidth:2,borderColor:'#b7a887',borderRadius:8,padding:8},servicesTitle:{color:'#624f3f',fontSize:6.5,fontWeight:'900'},servicesText:{color:'#7f6956',fontSize:10,fontWeight:'900',marginTop:15,lineHeight:24},
  ...
  panel:{position:'absolute',left:72,right:54,top:55,maxHeight:360,backgroundColor:'#f1eee3',borderRadius:12,borderWidth:3,borderColor:'#3f6474',padding:10,zIndex:200},panelTop:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginBottom:8},panelTitle:{color:'#294653',fontSize:12,fontWeight:'900'},close:{color:'#5d6f77',fontSize:15,fontWeight:'900'},panelLead:{color:'#3b5964',fontSize:7,fontWeight:'900',marginVertical:7},setRow:{flexDirection:'row',gap:6},setButton:{flex:1,backgroundColor:'#ded9cb',borderRadius:8,borderWidth:1,borderColor:'#a9a292',padding:8,alignItems:'center'},setButtonActive:{backgroundColor:'#c9ebd3',borderColor:'#55a874'},setButtonLocked:{opacity:.4},setText:{color:'#344e58',fontSize:7,fontWeight:'900'},setSub:{color:'#7a898e',fontSize:5.5,marginTop:2},listRow:{paddingVertical:9,paddingHorizontal:8,borderBottomWidth:1,borderBottomColor:'#ccc7b9'},listRowActive:{backgroundColor:'#fff5c9'},lockedRow:{opacity:.42},listTitle:{color:'#344e58',fontSize:7.2,fontWeight:'900'},listSub:{color:'#7a898e',fontSize:5.8,marginTop:3},flowRow:{flexDirection:'row',alignItems:'center',gap:6,paddingVertical:6},flowTrack:{flex:1,height:7,backgroundColor:'#d4d0c4',borderRadius:4,overflow:'hidden'},flowFill:{height:'100%',backgroundColor:'#5bb6dd'},flowFillHot:{backgroundColor:'#e66f5a'},flowPct:{width:32,color:'#586970',fontSize:6,fontWeight:'900',textAlign:'right'},
