...
const SAVE_KEY = 'rail-rush-hour-v013';
const LEGACY_SAVE_KEY = 'rail-rush-hour-v012';
const TICK_MS = 1000;
...
function TravelerStream({ left, top, width, amount, color, label }) {
  ...
  const count = Math.min(9, Math.max(1, Math.ceil(amount / 3)));
  const move = anim.interpolate({ inputRange: [0, 1], outputRange: [0, width - 25] });
  return (
    <View pointerEvents="none" style={[styles.flowOverlay, { left, top, width }]}>
      ...
      {Array.from({ length: count }).map((_, i) => (
        <Animated.View key={i} style={[styles.personSprite, { left: -12 - i * 20, transform: [{ translateX: move }] }]}>
...
function TrainSvg({ x, y, train, color, ready, late, onPress }) {
  ...
  return (
    <G onPress={onPress}>
      {Array.from({ length: train.sets }).map((_, i) => {
        const sx = x + i * (setWidth + gap);
        const body = late ? '#f3a0aa' : ready ? '#aef0c2' : '#e4eef2';
        return (
          <G key={i}>
            <Polygon points={`${sx},${y} ${sx + 50},${y - 12} ${sx + 62},${y - 3} ${sx + 12},${y + 9}`} fill="#081014" opacity="0.35" />
            <Polygon points={`${sx},${y - 6} ${sx + 50},${y - 18} ${sx + 62},${y - 9} ${sx + 12},${y + 3}`} fill={body} stroke={color} strokeWidth="2" />
            <Polygon points={`${sx},${y - 6} ${sx + 8},${y - 8} ${sx + 20},${y + 1} ${sx + 12},${y + 3}`} fill={color} />
            {[0, 1, 2, 3].map((w) => <Polygon key={w} points={`${sx + 22 + w * 8},${y - 12 - w * 1.9} ${sx + 28 + w * 8},${y - 13.4 - w * 1.9} ${sx + 31 + w * 8},${y - 10.8 - w * 1.9} ${sx + 25 + w * 8},${y - 9.4 - w * 1.9}`} fill="#315a70" />)}
            <SvgText x={sx + 34} y={y - 1} fontSize="7" fontWeight="900" fill="#183848">{train.type.code}</SvgText>
          </G>
        );
      })}
    </G>
  ...
  return (
    <View style={styles.sceneCanvas}>
      <Svg width={WORLD_WIDTH} height={WORLD_HEIGHT} viewBox={`0 0 ${WORLD_WIDTH} ${WORLD_HEIGHT}`}>
        ...
        <Polygon points="0,420 1420,160 1420,520 0,520" fill="#98ba79" />
        <Polygon points="0,395 1420,135 1420,235 0,495" fill="#5d6667" />
        <Line x1="0" y1="430" x2="1420" y2="170" stroke="#d7dcda" strokeWidth="3" strokeDasharray="18 13" opacity="0.5" />

        {[70, 150, 230, 465, 520, 600, 1280, 1330].map((x, i) => <Tree key={i} x={x} y={i % 2 ? 360 : 335} scale={0.8 + (i % 3) * 0.12} />)}
        ...
        {Array.from({ length: parkingSlots }).map((_, i) => {
          ...
          return (
            <G key={i}>
              ...
              {i < occupiedCars ? <Car x={x + 2} y={y + 1} color={i % 4 === 0 ? '#ef6d62' : i % 4 === 1 ? '#4aa8ff' : i % 4 === 2 ? '#efc64f' : '#e8eef0'} /> : null}
            </G>
          );
        })}
        ...
        {[1, 2, 3].map((lane) => {
          ...
          return (
            <G key={lane}>
              ...
              {!locked && train ? <TrainSvg x={1080} y={y + 60} train={train} color={color} ready={ready} late={late} onPress={() => onDepart(lane)} /> : null}
              {!locked && train ? <G><Rect x="1080" y={y + 67} width="160" height="21" rx="5" fill="#0a171d" opacity="0.92" /><SvgText x="1088" y={y + 81} fontSize="7" fontWeight="900" fill={ready ? '#62e895' : late ? '#ff8a78' : '#d9e5e9'}>{train.number} • {train.status === 'ready' ? (depIn > 0 ? `VERTREK OVER ${depIn}s` : depIn >= -DELAY_MARGIN ? `VERTREK • ${DELAY_MARGIN + depIn}s MARGE` : `+${Math.abs(depIn + DELAY_MARGIN)}s TE LAAT`) : `${train.remaining}s HALTE`}</SvgText></G> : null}
              {!locked ? Array.from({ length: Math.min(18, Math.ceil(waiting / 12)) }).map((_, i) => <TinyPersonSvg key={i} x={1055 + (i % 9) * 27 + Math.floor(i / 9) * 8} y={y + 19 - (i % 9) * 6 + Math.floor(i / 9) * 20} color={i % 4 === 0 ? color : '#627f8e'} />) : null}
      ...
      <View style={styles.sceneLegend}>
        ...
        <Text style={styles.sceneLegendText}>👥 {parkingQueue + entranceQueue + hallTotal + sum(platformDemand)} in systeem</Text>
      </View>
    </View>
  );
}
...
export default function App() {
  ...
  const tryArrival = () => {
    ...
    if (!platformsRef.current[planned]) {
      ...
      setTimeout(() => {
        syncPlatforms({ ...platformsRef.current, [planned]: { ...train, status: 'dwelling', actualLane: planned, remaining: train.type.dwell } });
        updateService(train.id, { status: 'platform', actualLane: planned });
        arrivalBusy.current = false;
        setMessage(`${train.number} rijdt automatisch P${planned} binnen.`);
      }, 2200);
    }
  };
  ...
  const divert = (lane) => {
    ...
    setMessage(`${train.number}: P${train.plannedLane} → P${lane}. Reizigers verplaatsen mee.`);
    setTimeout(() => {
  ...
  const begin = () => {
    ...
    setNow(0);
    setMessage('Swipe door de isometrische wereld. Kijk waar reizigers ophopen en breid precies die schakel uit.');
    setPhase('playing');
  };
  ...
  const buildP3 = () => {
    ...
    setPlatform3(true);
    setMessage('Perron 3 gebouwd: bouwterrein verandert direct in een volwaardig perron.');
    persist();
  };
    ...
    return (
      <SafeAreaView style={styles.screen}>
        ...
        <View style={styles.menu}>
          <Text style={styles.kicker}>ISOMETRIC ASSETS / V0.13</Text>
          <Text style={styles.logo}>RAIL{`\n`}RUSH HOUR</Text>
          <Text style={styles.subtitle}>Een levende, scrollbare stationwereld met echte isometrische parkeerplaatsen, gebouwen, winkels, reizigers, perrons en treinen. Groei blijft zichtbaar én operationeel.</Text>
  ...
  return (
    <SafeAreaView style={styles.screen}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.worldFrame}>
          <View style={styles.worldFrameHead}>
            <View><Text style={styles.worldKicker}>V0.13 • LEVENDE ISOMETRISCHE WERELD</Text><Text style={styles.worldTitle}>SWIPE DOOR JE STATION</Text></View>
      ...
      <View style={styles.footer}><Text style={styles.footerText}>V0.13 • SVG ISOMETRISCHE ASSETS • SWIPE WERELD • TIK GROENE TREIN</Text></View>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  flowOverlay: { position: 'absolute', height: 62, zIndex: 15, overflow: 'hidden' },
  ...
  sceneLegend: { position: 'absolute', left: 12, bottom: 10, flexDirection: 'row', gap: 7 },
  ...
  serviceStatus: { width: 74, color: '#c5d1d7', fontSize: 6.3, fontWeight: '900', textAlign: 'right' },
